...
t = str(target_ip.get())
# po = str(port.get())
result = ("The target ip address {}.".format(t)) #and the port number is {p} '
...
target = t #
target_message = ("Testing visibility of northbound interface on host{}. ".format(target))
...
rs_message = ("Got {} for {}.".format(reqStatus,defaultGuiURLs[u]))
rs_label.config(text=rs_message)
ug_message = ("URL associated with {} GUI interface.".format(u))
ug_label.config(text=ug_message)
else:
if(verbose == True):
sg_message=('Got sheet{}for URL {}.'.format(reqStatus,u))
sg_label.config(text=sg_message)
except Exception as e:
if(verbose == True):
error_message=('Error testing URL: {}.'.format(e))
